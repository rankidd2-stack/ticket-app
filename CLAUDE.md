...
- **Theme toggle**: `src/components/ThemeToggle.tsx`, placed in normal document flow, not fixed-position — it lives in the Sidebar (bottom, next to Log out) for dashboard pages and next to the wordmark for login/signup. It was originally a global `fixed top-4 right-4` overlay; that collided with page-level content that also wants the top-right corner (e.g. the Assets filter tabs), so it moved into each layout's actual header/footer instead. Toggles a `.dark` class on `<html>` via `document.startViewTransition` (a left-to-right wipe; instant fallback if unsupported), persisted to `localStorage`. A no-flash inline script in `layout.tsx` reads the stored preference before paint. Uses `flushSync` for the React state update inside the transition callback — the View Transitions API requires the DOM to be fully settled before the callback returns, and a plain `setState` lands on React's own tick, too late.
